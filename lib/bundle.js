...
moduleBrowserify.exportsBrowserify = { // {{{
   "001" : {
      "name" : "rpl_welcome",
      "type" : "reply"
   },
...
},{}],2:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
var process=requireBrowserify("__browserify_process"),Buffer=requireBrowserify("__browserify_Buffer").Buffer;/*
...
exportsBrowserify.Client = Client;
var util = requireBrowserify('util');
//var mozSocket = requireBrowserify('./mozSocket').mozSocket

var replyFor = requireBrowserify('./codes');
        ...
        switch ( message.command ) {
            ...
            case "err_umodeunknownflag":
                if ( self.opt.showErrors )
                    console.log("\033[01;31mERROR: " + util.inspect(message) + "\033[0m");
                break;
            default:
                if ( message.commandType == 'error' ) {
                    ...
                    if ( self.opt.debug )
                        console.log("\033[01;31mUnhandled message: " + util.inspect(message) + "\033[0m");
                }
                break;
        }
    }); // }}}
...
//moduleBrowserify.exportsBrowserify.Client = 'toto';//Client;
...
Client.prototype.connect = function ( retryCount, callback ) { // {{{
    ...
    if (self.opt.secure) {
        ...
        //self.conn = net.createConnection(self.opt.port, self.opt.server);
            Connection = function(){};
            //util.inherits(Socket, mozSocket);
            ...
            connection.socket.connect(self.opt.server, self.opt.port);//'chat.freenode.net',6667) ;
    ...
    self.conn.addListener("end", function() {
        if ( self.opt.debug )
            console.log('Connection got "end" event');
    });
    self.conn.addListener("close", function() {
        ...
        if ( self.opt.retryCount !== null && retryCount >= self.opt.retryCount ) {
            if ( self.opt.debug ) {
                console.log( 'Maximum retry count (' + self.opt.retryCount + ') reached. Aborting' );
            }
        ...
        if ( self.opt.debug ) {
            console.log( 'Waiting ' + self.opt.retryDelay + 'ms before retrying' );
        }
        setTimeout( function() {
...
Client.prototype.send = function(command) { // {{{
    ...
    if ( ! this.conn.requestedDisconnect ) {
        this.conn.socket.send(args.join(" ") + "\r\n");
    }
}; // }}}
...
exportsBrowserify.parseMessage = parseMessage;

},{"./codes":1,"__browserify_Buffer":5,"__browserify_process":6,"util":4}],3:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
...
},{"__browserify_process":6}],4:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
...
},{"events":3}],5:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
requireBrowserify=(function(e,t,n,r){function i(r){if(!n[r]){if(!t[r]){if(e)return e(r);throw new Error("Cannot find moduleBrowserify '"+r+"'")}var s=n[r]={exportsBrowserify:{}};t[r][0](function(e){var n=t[r][1][e];return i(n?n:e)},s,s.exportsBrowserify)}return n[r].exportsBrowserify}for(var s=0;s<r.length;s++)i(r[s]);return i})(typeof requireBrowserify!=="undefined"&&requireBrowserify,{1:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
// UTILITY
var util = requireBrowserify('util');
var Buffer = requireBrowserify("buffer").Buffer;
...
// 1. The assert moduleBrowserify provides functions that throw
// AssertionError's when particular conditions are not met. The
// assert moduleBrowserify must conform to the following interface.

var assert = moduleBrowserify.exportsBrowserify = ok;
...
// understood by the spec. Implementations or sub moduleBrowserifys can pass
...
// EXTENSION! This is annoying to write outside this moduleBrowserify.
...
},{"util":2,"buffer":3}],2:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
var events = requireBrowserify('events');
...
},{"events":4}],5:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
exportsBrowserify.readIEEE754 = function(buffer, offset, isBE, mLen, nBytes) {
  var e, m,
...
exportsBrowserify.writeIEEE754 = function(buffer, value, offset, isBE, mLen, nBytes) {
  var e, m, c,
      eLen = nBytes * 8 - mLen - 1,
      eMax = (1 << eLen) - 1,
      eBias = eMax >> 1,
...
},{}],6:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
...
},{}],4:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
...
})(requireBrowserify("__browserify_process"))
},{"__browserify_process":6}],"buffer-browserify":[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
moduleBrowserify.exportsBrowserify=requireBrowserify('q9TxCC');
},{}],"q9TxCC":[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
function SlowBuffer (size) {
    this.length = size;
};

var assert = requireBrowserify('assert');

exportsBrowserify.INSPECT_MAX_BYTES = 50;
...
function base64ToBytes(str) {
  return requireBrowserify("base64-js").toByteArray(str);
}
...
SlowBuffer.prototype.base64Slice = function (start, end) {
  ...
  return requireBrowserify("base64-js").fromByteArray(bytes);
}
...
SlowBuffer.prototype.inspect = function() {
  ...
  for (var i = 0; i < len; i++) {
    out[i] = toHex(this[i]);
    if (i == exportsBrowserify.INSPECT_MAX_BYTES) {
      out[i + 1] = '...';
      break;
    }
  }
...
exportsBrowserify.SlowBuffer = SlowBuffer;
exportsBrowserify.Buffer = Buffer;
...
function readFloat(buffer, offset, isBigEndian, noAssert) {
  ...
  return requireBrowserify('./buffer_ieee754').readIEEE754(buffer, offset, isBigEndian,
      23, 4);
}
...
function writeFloat(buffer, value, offset, isBigEndian, noAssert) {
  ...
  requireBrowserify('./buffer_ieee754').writeIEEE754(buffer, value, offset, isBigEndian,
      23, 4);
}
...
},{"assert":1,"./buffer_ieee754":5,"base64-js":7}],7:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
(function (exportsBrowserify) {
	'use strict';
	...
	moduleBrowserify.exportsBrowserify.toByteArray = b64ToByteArray;
	moduleBrowserify.exportsBrowserify.fromByteArray = uint8ToBase64;
}());

},{}],8:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
exportsBrowserify.readIEEE754 = function(buffer, offset, isBE, mLen, nBytes) {
  var e, m,
      eLen = nBytes * 8 - mLen - 1,
      eMax = (1 << eLen) - 1,
...
},{}],3:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
function SlowBuffer (size) {
    this.length = size;
};
...
},{"assert":1,"./buffer_ieee754":8,"base64-js":9}],9:[function(requireBrowserify,moduleBrowserify,exportsBrowserify){
(function (exportsBrowserify) {
	'use strict';
...
;;moduleBrowserify.exportsBrowserify=requireBrowserify("buffer-browserify")
