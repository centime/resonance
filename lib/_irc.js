        ...
        switch ( message.command ) {
            ...
            case "NOTICE":
                ...
                if ( self.opt.debug && to == self.nick )
                    _log('GOT NOTICE from ' + (from?'"'+from+'"':'the server') + ': "' + text + '"');
                break;
            case "MODE":
                if ( self.opt.debug )
                    _log("MODE: " + message.args[0] + " sets mode: " + message.args[1]);
            ...
            case "NICK":
                ...
                if ( self.opt.debug )
                    _log("NICK: " + message.nick + " changes nick to " + message.args[0]);
            ...
            case "PRIVMSG":
                ...
                if ( self.opt.debug && to == self.nick )
                    _log('GOT MESSAGE from ' + from + ': ' + text);
                break;
            case "INVITE":
                var from = message.nick;
                var to   = message.args[0];
            ...
            case "QUIT":
                if ( self.opt.debug )
                    _log("QUIT: " + message.prefix + " " + message.args.join(" "));
                if ( self.nick == message.nick ) {
                    // TODO handle?
                    break;
                }
            ...
            case "err_umodeunknownflag":
                if ( self.opt.showErrors )
                    _log("\033[01;31mERROR: " + (message) + "\033[0m");
                break;
            default:
                if ( message.commandType == 'error' ) {
                    ...
                    if ( self.opt.debug )
                        _log("\033[01;31mUnhandled message: " + (message) + "\033[0m");
                }
                break;
        }
    }); // }}}
           ...
           if (self.conn.authorized ||
              ...
              self.conn.setEncoding('utf-8');
                if ( self.opt.certExpired &&
                   self.conn.authorizationError === 'CERT_HAS_EXPIRED' ) {
                     _log('Connecting to server with expired certificate');
                }
                if ( self.opt.password !==  null ) {
                    self.send( "PASS", self.opt.password );
                }
                if ( self.opt.debug )
                    _log('Sending irc NICK/USER');
             ...
             _log(self.conn.authorizationError);
           }
        });
    */
    }else {
    ...
    self.conn.addListener("end", function() {
        if ( self.opt.debug )
            _log('Connection got "end" event');
    });
    self.conn.addListener("close", function() {
        if ( self.opt.debug )
            _log('Connection got "close" event');
        if ( self.conn.requestedDisconnect )
            return;
        if ( self.opt.debug )
            _log('Disconnected: reconnecting');
        if ( self.opt.retryCount !== null && retryCount >= self.opt.retryCount ) {
            if ( self.opt.debug ) {
                _log( 'Maximum retry count (' + self.opt.retryCount + ') reached. Aborting' );
            }
        ...
        if ( self.opt.debug ) {
            _log( 'Waiting ' + self.opt.retryDelay + 'ms before retrying' );
        }
        setTimeout( function() {
