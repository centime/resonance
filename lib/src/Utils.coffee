...
names = 
  "guests":["Centime", "Zardov", "Beranger", "Arkenstone", "Sayat", "Kevlar", "Karas", "Victoria"]
...
randomSubString = (str) ->
    ...
    if n < 2
        n = 2
    i = Math.round(Math.random()*(max-n))
    str.substr(i,n)
...
getRandomName = () ->
    pre = ''
    pre += randomSubString(pickInArray(names.female))
    pre += randomSubString(pickInArray(names.guests))
    pre += randomSubString(pickInArray(names.male))
    
    post = ''
    post += randomSubString(pickInArray(names.female))
