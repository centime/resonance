...
function sendPageLeft(url){
  if ( typeof(url) != 'undefined'){
    console.log('/left '+url);
    if (NO_IRC === false)
      client.say('Resonance-bot','/leave '+url)
  }
}
tabs.on('close', function(tab) {
  sendPageLeft(tab.url);
});
...
tabs.on('ready', function(tab) {
  ...
  var chan = '#'+sha1(tab.url.host+tab.title).toString() ;
    ...
    // AAaAAAaAaaaarg
    // tofix 2 persons when someone is there with 2 tabs
  t_=[];
...
if (NO_IRC === false ){
  ...
  console.log('[[ CHAN ]] : '+chan);
  client.say('Resonance-bot','/enter '+url)
}//endif NO_IRC
