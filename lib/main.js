...
tabs.on('ready', function(tab) {
  ...
  var chan = '#'+sha1(tab.url.host+tab.title).toString() ;
    // AAaAAAaAaaaarg
  console.log(tabToPrevious[tab])
  tabToPrevious[tab] = tab.url ;
...
/*
function getWorker(tab,channelsToWorkers){
  console.log(channelsToWorkers);
  for (i in channelsToWorkers){
    console.log(i)
    if (channelsToWorkers[i].tab === tab){
      console.log('ok !');
    };
  }
  console.log(tab.title);
}
tabs.on("ready", function(tab){getWorker(tab,channelsToWorkers)});
